...
class Account:
    ...
    def find_by_id(cls, account_id):
        ...
        #sql command to ge all data from the account table
        ...
        conn.close()
        if row:
            return cls(account_number=str(row[1]), account_balance=row[2], account_type=str(row[3]), date_opened=row[4], id=row[0])
        return None
