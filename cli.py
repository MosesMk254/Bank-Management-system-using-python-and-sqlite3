...
def get_customer_by_id():
    try:
        customer_id = int(input('Enter customer ID: ')) #creating an input that accepts only integers
        customer = Customer.find_by_id(customer_id) #finding the customers id
        #checking wether the Id matches a customer in the db and if yes displaying their information and if no dispaying the error message
        if customer:
            print(f'ID: {customer.id}')
            print(f'Name: {customer.name}')
            print(f'Address: {customer.address}')
